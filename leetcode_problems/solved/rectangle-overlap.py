#https://leetcode.com/problems/rectangle-overlap/description/

# An axis-aligned rectangle is represented as a list [x1, y1, x2, y2], where (x1, y1) 
# is the coordinate of its bottom-left corner, and (x2, y2) is the coordinate of its 
# top-right corner. Its top and bottom edges are parallel to the X-axis, and its left 
# and right edges are parallel to the Y-axis.

# Two rectangles overlap if the area of their intersection is positive. To be clear, 
# two rectangles that only touch at the corner or edges do not overlap.

# Given two axis-aligned rectangles rec1 and rec2, return true if they overlap, 
# otherwise return false.

from typing import List

class Solution:
    def isRectangleOverlap(self, rec1: List[int], rec2: List[int]) -> bool:
        x1, y1, x2, y2 = rec1
        x3, y3, x4, y4 = rec2
        
        if x1 < x4 and y1 < y4 and x2 > x3 and y2 > y3:
            return True
        else:
            return False
 
sol1 = Solution()
print(sol1.isRectangleOverlap([0,0,2,2], [1,1,3,3]))
print(sol1.isRectangleOverlap([0,0,1,1], [1,0,2,1]))
print(sol1.isRectangleOverlap([0,0,1,1], [2,2,3,3]))  
print(sol1.isRectangleOverlap([7,8,13,15], [10,8,12,20]))        
#time - O(1)
#space - O(1)